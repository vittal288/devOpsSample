/*describe('mocking service http call', function() {
    ...
    var billController, $scope, myService, $httpBackend;
    var data = {
 "screen1": {
   ...
   "others": [{
    ...
    /*describe('with spies', function () {
        beforeEach(inject(function ($controller, $rootScope, _myService_) {
            ...
            controller = $controller('billcontroller', {$scope: $scope, myService: myService});
 ...
 billController = $controller('billcontroller', {$scope: $scope, myService: myService});
 }));

    it('to get json data', function () {
        expect($scope.data).toEqual(data);
        expect($scope.entiredata).toEqual(data.screen1);
        expect($scope.bill).toEqual(data.Hotels[$scope.HotelIndex].bill);
        expect($scope.others).toEqual(data.Hotels[$scope.HotelIndex].others);
    });
 });*/
   ...
   /* describe('with httpBackend', function () {
        ...
        $httpBackend.when('GET', 'app/view/login.html').respond(true);
        $httpBackend.when('GET', 'data/data.json').respond(data);

        billController = $controller('billcontroller', {$scope: $scope});
        $httpBackend.flush();
     }));
        ...
        it('to get json data', function () {
            expect($scope.data).toEqual(data);
            expect($scope.entiredata).toEqual(data.screen1);
            expect($scope.bill).toEqual(Hotels[0].bill);
            expect($scope.others).toEqual(Hotels[0].others);
    });
